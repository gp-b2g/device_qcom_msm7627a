...
#define ENABLE_ALS_SENSOR 	"/sys/bus/i2c/devices/1-0039/enable_als_sensor"
/*****************************************************************************/
...
LightSensor::LightSensor()
    : SensorBase(NULL, "Avago light sensor"),
      mEnabled(0),
...
bool LightSensor::isEnabled(int32_t handle) {
	return true;
}
...
int LightSensor::enable(int32_t handle, int en)
{
    ...
    if (flags != mEnabled) {
        int fd;
	  fd = open(ENABLE_ALS_SENSOR, O_RDWR);
        if (fd >= 0) {
            char buf[2];
            int err;
            buf[1] = 0;
